...
*discrete*, i.e. a vector of ±1, so each value names a phase. The trick is that the
...
500 ms, squeezed from both sides. Not shorter than the controller's own work, i.e. a
training step (~100 ms measured), a phase inference (< 5 ms) and an action
inference (< 2 ms), plus enough time for the chunk counter to advance measurably. Not
